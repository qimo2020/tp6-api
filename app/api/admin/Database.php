...
class Database extends Common
{
    ...
    private function getFields($table)
    {
        ...
        $res = Db::query("SELECT COLUMN_NAME FROM information_schema.columns WHERE TABLE_NAME='{$prefix}" . $table . "' and TABLE_SCHEMA='".config('database.connections.mysql.database')."'");
    ...
    public function import()
    {
        try{
            if ($this->request->isPost()) {
                ...
                return $this->response(1, '导入成功', (string)url('', ['app_id'=>$appId]));
            }
        }catch (\Exception $e) {
            return $this->response(0, $e->getMessage());
        }
